...
function setup() {
  ...
  input.style('background-color', '#fcc1e3ff'); // 設定輸入框背景顏色
  input.style('color', '#ff98f5ff');            // 設定輸入框文字顏色
  ...
  jumpSel = createSelect(); // 建立跳動模式選單
  jumpSel.position(button.x + button.width + 10, 20);
  jumpSel.option('波浪');
  jumpSel.option('左右');
  jumpSel.option('隨機');
  jumpSel.style('border-radius', '10px');
  jumpSel.style('font-size', '14px');
  ...
  sel.position(jumpSel.x + jumpSel.width + 50, 20); // 定位在跳動選單右側
...
function draw() {
  ...
  let colors = ['#d35ff3ff', '#a4bdddff', '#d05cddff', '#68c2d8ff', '#1e6091']; // 色票陣列
  ...
  if (txt.length > 0) {
    ...
    for (let y = 100; y < height; y += slider.value() * 1.5) {
      ...
      for (let x = 0; x < width; x += w + 30) {
        ...
        if (isJumping) {
          let mode = jumpSel.value();
          if (mode === '波浪') {
            text(txt, x, y + sin(frameCount * 0.05 + x * 0.02) * 10); // 溫和波浪效果
          } else if (mode === '左右') {
            text(txt, x + sin(frameCount * 0.1 + y * 0.01) * 10, y); // 左右搖擺
          } else if (mode === '隨機') {
            text(txt, x + random(-3, 3), y + random(-3, 3)); // 隨機震動
          }
        } else {
          text(txt, x, y);
        }
